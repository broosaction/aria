...
class BuildRoutes extends Builder
{
    ...
    public function build()
    {
        ...
        foreach (Finder::findFiles('*.php', '*.properties')
                     ->from($this->dir.'/app/Bootstrap/Controllers') as $file) {
       ...
       $cnt =  "<?php \n" . $namespace ." ".$code;
       FileSystem::write($this->dir.'/App/Bootstrap/Routes/web.php', $cnt);
        ...
        foreach ($_classes as $k => $v){
            ...
            foreach ($cl->getMethods() as $method){
                ...
                if($props->getMethod() !== null){
                    ...
                    $code .= "\t\t Router::match(".$props->getMethod().", '".$props->getPath()."', '".$v."@".$method->getName()."', ".$props->getRouteProps().")->setName('".$props->getName()."'); \n";
                    $this->output->writeln('Added: '.$method->getName().'  listening on: '.$props->getMethod().$props->getPath());
                    $this->counter++;
                }
    ...
    private function prepare($_file){
        $file = str_replace($this->dir.'/app/Bootstrap/','App\Bootstrap\\',$_file);
