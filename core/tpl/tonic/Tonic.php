...
class Tonic{
     ...
     */
    public static $context_aware = false;
    /**
    ...
    public function set_themes_dir($dir){
        $this->themes_dir = $dir;
        $this->cache_dir = $this->themes_dir.'/'.$this->cache_dir;
    }
    /**
    ...
    private function saveCache(){
        ...
        $cache=@fopen($this->cache_dir.$file_name, 'wb');
    ...
    private function handleVars(){
        ...
        if(!empty($matches)){
            foreach($matches[1] as $i => $var_name){
                ...
                }else if(strpos($real_var, 'server_home') !== false){
                    $this->content=$this->str_replace_first($matches[0][$i],aria()->getServerHome(),$this->content);
                }else{
