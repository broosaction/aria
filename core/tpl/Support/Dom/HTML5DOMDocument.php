 ...
 *
 * local repository is modified;
 * Bruce Mubangwa <bruce@broosaction.com>
 */

namespace Core\tpl\Support\Dom;

use Core\tpl\Support\Dom\Internal\QuerySelectors;

/**
 * Represents a live (can be manipulated) representation of a HTML5 document.
 *
 * @method \Core\tpl\Support\Dom\HTML5DOMElement|false createElement(string $localName, string $value = '') Create new element node.
 * @method \Core\tpl\Support\Dom\HTML5DOMElement|false createElementNS(?string $namespace, string $qualifiedName, string $value = '') Create new element node with an associated namespace.
...
class HTML5DOMDocument extends \DOMDocument
{
    ...
    public function __construct(string $version = '1.0', string $encoding = '')
    {
        ...
        $this->registerNodeClass('DOMElement', 'Core\tpl\Support\Dom\HTML5DOMElement');
    }
    ...
    public function modify($modifications = 0)
    {
        ...
        foreach ($headElements as $headElement) {
            ...
            if ($fixDuplicateMetatags) { // Remove all meta tags that has matching name or property attributes.
                ...
                $metaTags = $bodyElements->item(0)->getElementsByTagName('meta');
                if ($metaTags->length > 0) {
                    foreach ($metaTags as $metaTag) {
                        $headElements->item(0)->appendChild($metaTag);
                    }
                }
