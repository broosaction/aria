...
class InitExtentions implements TemplateHandler
{
    ...
    public function handle(&$content, AriaCompiler $compiler, $lang = 'PHP')
    {
        ...
        $compiler->composer('random_color', static function ($input) {

            return RandomColor::many(36, array('luminosity' => 'random', 'hue' => 'random', 'format' => 'rgbCss'))[0];
        });

        $compiler->composer('random_dark_color', static function ($input) {
            return RandomColor::many(27, array('luminosity' => 'dark', 'format' => 'rgbCss'));
        });
        ...
        $compiler->composer('default', static function ($input, $val) {
            if (is_null($val)) {
        ...
        $compiler->composer('get_initial', static function ($input) {
            $name_slice = explode(' ', $input);
        ...
        $compiler->composer('is', static function ($input, $true_val, $return) {
            if (empty($true_val)) {
                throw new \Exception('true value is required');
            }
            if ($input === $true_val) {
