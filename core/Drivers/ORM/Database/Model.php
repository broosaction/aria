...
use PDO;
use Core\Drivers\ORM\Observer\{ModelObserver,ObserverSubject};
use Core\Drivers\ORM\Pagination\Paginate;
...
abstract class Model{
	...
	protected function connectDatabase(){
        ...
        return (new Connector)->getPDO([
            'driver' => 'mysql',
            'dbname' => aria()->getConfig()->db_database,
            'charset' => 'utf8mb4',
            'collation' => 'utf8mb4_unicode_ci',
            'host' => aria()->getConfig()->db_host,
            'user_name' => aria()->getConfig()->db_username,
            'user_password' => aria()->getConfig()->db_password
        ]);
	...
	if(isset(self::${$where}[self::$currentField.self::$currentSubQueryNumber])){
		...
		if($current['havingNumber']!==NULL){
			foreach (range(0, $current['havingNumber']-1) as $key => $value) {
				$result=self::$currentField['havingField'][$key] . ' ' . self::$currentField['havingOperator'][$key] . ' ' .self::$currentField['havingValue'][$key];
				$string .=$key==0 ? ' HAVING ' . $result : ' AND ' . $result;
			}
		}
	}
