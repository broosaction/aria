...
use Core\Joi\System\Exceptions\DownloadException;
use Core\Joi\System\Exceptions\ParserException;
...
class AutoUpdate {
    ...
    private \Nette\Caching\Cache|\Core\Drivers\Nette\Caching\Cache $cache;
     ...
     */
    public function __construct(Start $server,?string $tempDir = null, ?string $installDir = null, int $maxExecutionTime = 60)
    {
        //Init logger
     ...
     * @return AutoUpdate
     */
    public function setUpdateFile(string $updateFile): self
    {
     ...
     * @return AutoUpdate
     */
    public function setSslVerifyHost(bool $sslVerifyHost): self
    {
    ...
    public function checkUpdate(int $timeout = 10)
    {
        ...
        $versions = $this->cache->load('update-versions');
        ...
        if ($versions === null || $versions === false) {
            ...
            $this->cache->save('update-versions', $versions, [
                aria()->getCache()::EXPIRE => '12 hours', // accepts also seconds or a timestamp.
                aria()->getCache()::SLIDING => false,
            ]);
        } else {
