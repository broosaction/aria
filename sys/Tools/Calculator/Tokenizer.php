...
class Tokenizer implements TokenizerInterface
{
    ...
    public function tokenize(string $expression, array $functionNames = []): array
    {
        ...
        for($i = 0; $i < $exprLength; $i++) {
            ...
            else if(in_array($expression[$i], Tokens::PARENTHESES, true)) {
                if($tokens && $expression[$i] === Tokens::PAREN_LEFT &&
                    (is_numeric($tokens[count($tokens) - 1]) || in_array($tokens[count($tokens) - 1], Tokens::PARENTHESES, true))) {
                    $tokens[] = Tokens::MULT;
                }

                $tokens[] = $expression[$i];
            }
            else if(in_array($expression[$i], Tokens::OPERATORS, true)) {
